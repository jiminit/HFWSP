...
def construct_life_table(sex_input, 
                         age_input, 
                         ldl_input, 
                         sbp_input, 
                         lpa_input, 
    ...
    if ldl_lowering_input_p is not None:
        df.loc[df['age'] >= age_input, 'ldl'] = df['ldl']*(1-ldl_lowering_input_p)

    if ldl_lowering_input_a is not None:
    ...
    #Calculate LSI across the lifetime
    ...
    #Quit smoking
    if quit_smoking_input is not None:
        smo = age_input
        df['dursmk'] = np.nan
        df['tsc'] = np.nan
        df.loc[df['age'] < sma_input, 'dursmk'] = 0
        df.loc[(df['age'] >= sma_input) & (df['age'] < smo), 'dursmk'] = df['age'] - sma_input
        df.loc[df['age'] >= smo, 'dursmk'] = smo - sma_input
        df.loc[df['age'] < smo, 'tsc'] = 0
        df.loc[df['age'] >= smo, 'tsc'] = df['age'] - smo
        if cig_input is None or np.isnan(cig_input):
            cig_input = 18
        df['lsi'] = 0.0
        df.loc[df['age'] >= sma_input, 'lsi'] = (1-(0.5**(df['dursmk']/18)))*(0.5**(df['tsc']/18))*np.log(cig_input+1)

    #Time-weight risk factors
    ages = df['age'].values 
    ldl = df['ldl'].values
    lpa = df['lpa'].values
    sbp = df['sbp'].values
